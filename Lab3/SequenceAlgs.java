...
public class SequenceAlgs<E> {
	...
	public boolean isPalindrome (Sequence<E> S) {
		...
		for (int i = 0; i < S.size(); i++) {
			int correspondingIndex = S.size() - (i + 1);
			
			// Stop when we've checked all pairs
			if (i >= correspondingIndex) {
				break;
			}
			
			// Compare elements at current index and corresponding index
			if (!S.get(i).equals(S.get(correspondingIndex))) {
				return false;
			}
		}
